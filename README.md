...
expected values. As an example, imagine we are writing functions to work
with an augmented version of the built-in data `state.x77` that contains
a column `state`, which should only have values that are present in the
`state` column of our template dataframe, and should contain *all* of
the entries from our template dataframe. Let’s imagine our template
...
We can generate tests for the presence of expected values and absense of
unexpected values with `testmaker_df_colcontents_tt` (or `..._sin` for
